...
def generate_review_report(xml_document, docx_fname, which="bdp"):
    ...
    _add_tag(document, "Metadata file being reviewed", fname, tag_style="fgdc bold")
    try:
        username = getpass.getuser()
        contact = utils.get_usgs_contact_info(username, True)
        reviewer_str = "{} ({})".format(
            contact["fgdc_cntperp"]["fgdc_cntper"], contact["fgdc_cntemail"]
        )
    except:
    ...
    _add_tag(document, "Review Date", time.strftime("%m/%d/%Y"), tag_style="fgdc bold")
    document.add_paragraph("")
