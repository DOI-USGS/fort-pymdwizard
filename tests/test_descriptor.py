...
def test_descriptor_from_xml(qtbot):
    ...
    assert (
        widget.findChild(QPlainTextEdit, "fgdc_abstract").toPlainText() == "Abstract MH"
    )
    assert (
        widget.findChild(QPlainTextEdit, "fgdc_purpose").toPlainText() == "Purpose MH"
    )
    assert (
        widget.findChild(QPlainTextEdit, "fgdc_supplinf").toPlainText()
        == "Supplemental Information MH"
    )
...
def test_descriptor_to_xml(qtbot):
    ...
    widget.findChild(QPlainTextEdit, "fgdc_abstract").setPlainText(
    ...
    assert (
        etree.tostring(mdDescript, pretty_print=True).decode()
        == """<descript>
